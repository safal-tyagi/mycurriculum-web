...
const CourseReader = () => {
  ...
  const handleChapterClick = (chapterNumber) => {
    const chapter = currentCourse.chapters.find(ch => ch.chapter_number === chapterNumber);
  ...
  const handleSectionClick = async (chapterNumber, sectionNumber) => {
    const chapter = currentCourse.chapters.find(ch => ch.chapter_number === chapterNumber);
    const section = chapter.sections.find(sec => sec.section_number === sectionNumber);
    ...
    if (section.content) {
      ...
      if (result.data) {
        ...
        const updatedSections = chapter.sections.map(sec =>
          sec.section_number === sectionNumber ? { ...sec, content: result.data.content } : sec
        );
        const updatedChapters = currentCourse.chapters.map(ch =>
          ch.chapter_number === chapterNumber ? { ...ch, sections: updatedSections } : ch
        );
        ...
        <List>
          {currentCourse && currentCourse.chapters.map((chapter) => (
            <React.Fragment key={chapter.chapter_number}>
              <ListItem button onClick={() => handleChapterClick(chapter.chapter_number)}>
                <ListItemText primary={chapter.chapter_name} />
                {currentChapter && currentChapter.chapter_number === chapter.chapter_number ? <ExpandLess /> : <ExpandMore />}
              </ListItem>
              <Collapse in={currentChapter && currentChapter.chapter_number === chapter.chapter_number} timeout="auto" unmountOnExit>
                <List component="div" disablePadding>
                  {chapter.sections.map((section) => (
                    <ListItem button key={section.section_number} onClick={() => handleSectionClick(chapter.chapter_number, section.section_number)}>
          ...
          <Box>
            ...
            <List>
              {currentCourse.chapters.map((chapter) => (
                <ListItem button key={chapter.chapter_number} onClick={() => handleChapterClick(chapter.chapter_number)}>
                  <ListItemText primary={chapter.chapter_name} />
                </ListItem>
              ))}
            </List>
          ...
          <Box>
            ...
            <List>
              {currentChapter.sections.map((section) => (
                <ListItem button key={section.section_number} onClick={() => handleSectionClick(currentChapter.chapter_number, section.section_number)}>
                  <ListItemText primary={section.section_name} />
                </ListItem>
              ))}
            </List>
